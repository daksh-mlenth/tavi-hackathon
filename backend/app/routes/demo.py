...
async def simulate_vendor_reply(
    request: SimulateVendorReplyRequest, db: Session = Depends(get_db)
):
    """
    ...
    if request.channel == "sms":
        await process_vendor_sms_response(
            db=db, quote_id=quote.id, vendor_id=vendor.id, message=request.reply_message
        )
        ...
        parsed = await ai_service.parse_vendor_phone_response(
            ...
            work_order_data={
                ...
                "trade_type": quote.work_order.trade_type.value
                if quote.work_order
                else "",
            },
        )
        ...
        currency_info = get_currency_info(
            quote.work_order.location_country if quote.work_order else "United States"
        )
        ...
        raise HTTPException(
            status_code=400,
            detail="Invalid channel. Must be 'sms', 'email', or 'phone'",
        )
...
async def simulate_multiple_vendor_quotes(
    ...
    for quote in quotes[:5]:
        ...
        await process_vendor_sms_response(
            db=db, quote_id=quote.id, vendor_id=quote.vendor_id, message=vendor_message
        )
...
async def simulate_facility_manager_confirmation(
    request: SimulateFacilityConfirmationRequest, db: Session = Depends(get_db)
):
    """Simulate facility manager approving/rejecting vendor selection"""

    work_order = (
        db.query(WorkOrder).filter(WorkOrder.id == UUID(request.work_order_id)).first()
    )
    if not work_order:
        raise HTTPException(status_code=404, detail="Work order not found")

    response_text = (
        "APPROVED - Vendor selection looks good!"
        if request.approved
        else "REJECTED - Please select another vendor"
    )
...
async def simulate_vendor_dispatch_confirmation(
    request: SimulateVendorDispatchRequest, db: Session = Depends(get_db)
):
    ...
    response_text = (
        "CONFIRMED - I will be there on the scheduled date!"
        if request.confirmed
        else "Sorry, I cannot make it on that date. Can we reschedule?"
    )
