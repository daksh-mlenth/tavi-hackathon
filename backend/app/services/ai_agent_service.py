...
Tavi Team"""
    ...
    async def parse_vendor_response(self, response_text: str) -> Dict[str, Any]:
        ...
        try:
            response = await self.client.chat.completions.create(
                model=AI_MODEL,
                messages=[
                    {"role": "system", "content": VENDOR_RESPONSE_PARSING_SYSTEM_PROMPT},
                    {"role": "user", "content": VENDOR_RESPONSE_PARSING_USER_PROMPT(response_text)}
                ],
                response_format=RESPONSE_FORMAT_JSON,
                temperature=AI_TEMPERATURE_PARSING
            )
    ...
    async def parse_vendor_email_response(
        ...
        if not self.client:
            return {"needs_human": True, "reason": "No AI client", "draft_response": "Thank you for your response."}
        ...
        try:
            ...
            response = await self.client.chat.completions.create(
                model=AI_MODEL,
                messages=[
                    {"role": "system", "content": "You are a professional service coordinator."},
    ...
    async def parse_vendor_phone_response(
        ...
        try:
            ...
            response = await self.client.chat.completions.create(
                model=AI_MODEL,
                messages=[
                    {"role": "system", "content": "You parse call transcripts and extract quote info as JSON."},
                    {"role": "user", "content": prompt}
                ],
                temperature=AI_TEMPERATURE_PARSING
            )
    ...
    async def parse_vendor_sms_response(
        ...
        if not self.client:
            return {"needs_human": True, "reason": "No AI client", "response": "Thanks! Reviewing."}
        ...
        try:
            ...
            response = await self.client.chat.completions.create(
                model=AI_MODEL,
                messages=[
                    {"role": "system", "content": "You are a service coordinator texting a vendor."},
                    {"role": "user", "content": prompt}
                ],
                response_format=RESPONSE_FORMAT_JSON,
                temperature=AI_TEMPERATURE_GENERATION
    ...
    async def generate_negotiation_message(
        ...
        try:
            ...
            return f"Thank you for your quote. Can we discuss pricing?"
